...
Repository::~Repository() {
    saveToFile();
    // Free dynamically allocated Commit objects
...
// Encode newlines so content stays on one physical line in the data file
...
void Repository::loadFromFile() {
    ...
    for (int i = 0; i < cCount; ++i) {
        std::getline(file, line);   // COMMIT_START
        ...
        for (int j = 0; j < cfcount; ++j) {
            std::getline(file, line);   // CFILE_START
            ...
            std::getline(file, line);   // CFILE_END
            cfiles.emplace_back(cfname, cfcontent);
        }

        // Dynamic memory: Commit objects live on the heap
        commits.push_back(new Commit(cid, cmsg, cts, cnum, cfiles));
        std::getline(file, line);   // COMMIT_END
    }
...
void Repository::addFile(const std::string& filename, const std::string& content) {
    ...
    if (idx != -1) {
        workingDirectory[idx].setContent(content);   // update existing
    } else {
        workingDirectory.emplace_back(filename, content);   // add new
    }
}
...
void Repository::commit(const std::string& message) {
    ...
    // Allocate on heap (dynamic memory requirement)
...
void Repository::showCommitDetail(int number) const {
    for (const Commit* c : commits) {
        if (c->getCommitNumber() == number) {
            // Virtual dispatch → Commit::displayDetailed() is called
            c->displayDetailed();
            return;
        }
    }
...
void Repository::searchCommits(const std::string& keyword) const {
    // Case-insensitive search — keyword and targets both lowercased
...
bool Repository::revert(int commitNumber) {
    for (const Commit* c : commits) {
        if (c->getCommitNumber() == commitNumber) {
            workingDirectory = c->getFiles();   // restore snapshot
            return true;
        }
    }
    return false;
